...
export default function App() {
  ...
  const [profile, setProfile] = useState<BioProfile>(() => {
    ...
    return {
      ...
      goals: { targetKcal: 2500, protein: 188, fat: 69, carbs: 281, proteinPct: 30, fatPct: 25, carbsPct: 45, currentGoal: 'maintain', correction: 0, weightStart: 85, weightTarget: 80 }
    };
  });
  ...
  // Initial Data Fetch from Cloud on Login
  useEffect(() => {
    if (user && db) {
      const fetchCloudData = async () => {
        ...
        const plansSnap = await getDoc(doc(db, "users", user.uid, "data", "plans"));
  ...
  // Fix: Added handleImageUpload for scanner
  ...
  // Fix: Added addScannedToDay for scanner
  ...
  const handleFridgeCreation = async () => {
    ...
    try {
      ...
      const recipe = await generateFridgeRecipe(fridgeString, 30, "Średnie", "Standard", {
      ...
      // Fallback display logic or result handling
    } catch (e: any) { setError(e.message); } finally { setLoading(false); }
  };
  ...
  if (authLoading) return <div className="min-h-screen bg-[#0a0a0b] flex items-center justify-center italic text-stone-700">Inicjalizacja Logistyki...</div>;
          ...
          <div className="space-y-10 animate-in slide-in-from-bottom duration-500">
            ...
            <section className="space-y-8 pb-10">
              ...
              {currentPlan.meals.map((m: Meal) => {
                const isExp = expandedMealIds.has(m.id);
                return (

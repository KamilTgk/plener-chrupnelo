...
export default function App() {
  ...
  const toggleMealCompletion = (idx: number) => {
    ...
    setPlans(prev => ({
        ...prev,
        [selectedDate]: { ...currentPlan, meals: updatedMeals }
    }));
  };
  ...
  const editWaterGoal = () => {
    ...
    if (val && !isNaN(parseInt(val))) {
        setWater(prev => ({ ...prev, target: Math.abs(parseInt(val)) }));
    }
  };
  ...
  const editStepsGoal = () => {
    ...
    if (val && !isNaN(parseInt(val))) {
        setSteps(prev => ({ ...prev, target: Math.abs(parseInt(val)) }));
    }
  };
  ...
  const setStrategy = (mode: 'cut' | 'maintain' | 'bulk' | 'custom', val: number) => {
    setProfile(prev => ({
        ...prev,
        goals: { ...prev.goals, currentGoal: mode, correction: val }
    }));
  };if (!user) {
    return (
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0b] text-white flex flex-col max-w-xl mx-auto pb-44 font-sans text-center selection:bg-[#ff7a00]/30 relative">
      <header className="p-10 flex flex-col items-center sticky top-0 bg-[#0a0a0b]/90 backdrop-blur-xl z-50 border-b border-white/5">
        ...
        <span className="text-[8px] font-black text-stone-600 tracking-[0.4em] uppercase mt-2 italic">Strategia Operacyjna AI</span>
            ...
            <div className="flex bg-[#161618] p-1 rounded-2xl border border-white/5 mb-4">
               {['day', 'week', 'month'].map((m) => (
                   <button key={m} onClick={() => setPlanViewMode(m as any)} className={`flex-1 py-2 rounded-xl text-[9px] font-black uppercase transition-all ${planViewMode === m ? 'bg-[#ff7a00] text-black' : 'text-stone-600'}`}>
                       {m === 'day' ? 'Dzień' : m === 'week' ? 'Tydzień' : 'Miesiąc (Archiwum)'}
                   </button>
               ))}
            </div>
            ...
            {planViewMode === 'day' && (
                <>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <input placeholder="Kuchnia świata..." value={cuisine} onChange={(e) => setCuisine(e.target.value)} className="w-full bg-[#0a0a0b] border border-white/5 p-4 rounded-2xl text-[10px] font-black italic outline-none focus:border-[#ff7a00] text-center" />
                    ...
                    <section className="bg-[#161618] p-8 rounded-[3.5rem] border border-[#27272a] space-y-6 text-left relative overflow-hidden">
                        <div className="flex justify-between items-end px-2 relative z-10">
                            ...
                            <div className="text-right">
                                <span className="text-stone-300 font-bold text-[10px]">{Math.round((consumedStats.kcal / calculated.totalTarget) * 100)}% CELU</span>
                            </div>
                        </div>
                    ...
                    {shoppingList.length > 0 && (
                        <section className="bg-[#161618] p-8 rounded-[3.5rem] border border-white/5 mt-8 text-left">
                             <h3 className="text-center text-[10px] font-black text-[#ff7a00] uppercase italic tracking-[0.2em] mb-6">Lista Zakupów na Dziś</h3>
                    ...
                    <section className="bg-[#161618] p-6 rounded-[3rem] border border-[#27272a] space-y-3">
                        <div className="flex justify-between items-center">
                             <span className="text-[8px] font-black text-stone-500 uppercase">🥛 Woda</span>
                             <button onClick={editWaterGoal} className="text-[7px] font-bold text-stone-600 hover:text-[#ff7a00] uppercase tracking-widest bg-[#0a0a0b] px-2 py-1 rounded-full border border-white/5">
                                 Cel: {water.target}
                             </button>
                        </div>
                        <div className="flex justify-between items-center px-2">
                            <button onClick={() => setWater({...water, current: Math.max(0, water.current - 250)})} className="text-stone-600 font-black text-xl hover:text-white transition-colors">-</button>
                            <div className="flex flex-col items-center leading-none">
                                <span className="text-sm font-black text-blue-400">{water.current}ml</span>
                                <span className="text-[7px] font-bold text-stone-600 mt-0.5">/ {water.target}ml</span>
                            </div>
                            <button onClick={() => setWater({...water, current: water.current + 250})} className="text-blue-400 font-black text-xl hover:text-blue-300 transition-colors">+</button>
                        </div>
                    </section>

                    <section className="bg-[#161618] p-6 rounded-[3rem] border border-[#27272a] space-y-3">
                        <div className="flex justify-between items-center">
                            <span className="text-[8px] font-black text-stone-500 uppercase">👣 Kroki</span>
                            <button onClick={editStepsGoal} className="text-[7px] font-bold text-stone-600 hover:text-[#ff7a00] uppercase tracking-widest bg-[#0a0a0b] px-2 py-1 rounded-full border border-white/5">
                                 Cel: {steps.target}
                            </button>
                        </div>
                        <div className="text-xs font-black text-green-400 flex items-baseline justify-center gap-1">
                            {steps.current} <span className="text-[8px] text-stone-600 font-bold">/ {steps.target}</span>
                        </div>
             ...
             {planViewMode === 'month' && (
                <div className="space-y-4">
                    <h3 className="text-center text-[10px] font-black text-stone-500 uppercase italic tracking-[0.2em]">Archiwum Operacyjne (30 dni)</h3>
                    {monthHistory.length === 0 && <p className="text-stone-600 text-xs italic py-10">Brak danych historycznych.</p>}
                    {monthHistory.map((day: any) => (
                        <div key={day.date} className="bg-[#161618] p-5 rounded-[2rem] border border-white/5 flex justify-between items-center">
                            <div>
                                <p className="text-[10px] font-black uppercase text-stone-500">{day.date}</p>
                                <p className="text-stone-300 font-bold text-xs">{day.plan.meals?.length} posiłków</p>
                            </div>
                            <div className="text-right">
                                <span className="block text-[#ff7a00] font-black italic">{day.consumed} kcal</span>
                                <span className="text-[8px] uppercase text-stone-600">Zjedzone</span>
                            </div>
                        </div>
                    ))}
                </div>
             ...
             {planViewMode === 'week' && (
                 <div className="py-20 text-stone-500 text-xs italic uppercase font-black">
                     Widok tygodniowy w przygotowaniu przez AI...
                 </div>
             )}
          </div>
        )}
              ...
              {scanResult && (
                <div className="bg-[#0a0a0b] p-6 rounded-[3rem] border border-[#ff7a00]/20 animate-in slide-in-from-top mt-4">
                  ...
                  <div className="grid grid-cols-4 gap-2 mt-4">
                    {[{l:'KCAL',v:scanResult.kcal,c:'white'},{l:'B',v:scanResult.protein,c:'blue-500'},{l:'T',v:scanResult.fat,c:'orange-500'},{l:'W',v:scanResult.carbs,c:'green-500'}].map(m=>(
                      <div key={m.l} className="bg-[#161618] p-3 rounded-xl text-center">
                        <span className={`text-[7px] font-black block text-${m.c}`}>{m.l}</span>
                        <span className="text-sm font-black">{m.v}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {activeTab === 'body' && (
          <div className="space-y-6 animate-in slide-in-from-right duration-500 pb-20">
            ...
            <section className="bg-[#161618] p-8 rounded-[4rem] border border-[#27272a] space-y-6">
              <h3 className="text-center text-[10px] font-black text-stone-500 uppercase italic tracking-[0.2em]">Parametry Biologiczne</h3>
              
              <div className="flex bg-[#0a0a0b] p-1 rounded-2xl border border-white/5">
                <button 
                  onClick={() => setProfile({...profile, bio: {...profile.bio, gender: 'male'}})} 
                  className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${profile.bio.gender === 'male' ? 'bg-[#3b82f6] text-black shadow-lg' : 'text-stone-600'}`}
                >
                  Mężczyzna
                </button>
                <button 
                  onClick={() => setProfile({...profile, bio: {...profile.bio, gender: 'female'}})} 
                  className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${profile.bio.gender === 'female' ? 'bg-[#ff7a00] text-black shadow-lg' : 'text-stone-600'}`}
                >
                  Kobieta
                </button>
              </div>
              ...
              <div className="space-y-8 px-2">
                <div className="space-y-2">
                   <div className="flex justify-between items-end">
                      <span className="text-[9px] font-black text-stone-500 uppercase">Waga</span>
                      <span className="text-xl font-black italic text-white">{profile.bio.weight} <small className="text-xs text-stone-600">kg</small></span>
                   </div>
                   <input type="range" min="40" max="150" step="0.5" value={profile.bio.weight} onChange={(e) => setProfile({...profile, bio: {...profile.bio, weight: parseFloat(e.target.value)}})} className="w-full h-2 bg-[#0a0a0b] rounded-full appearance-none accent-[#ff7a00] cursor-pointer" />
                </div>
                <div className="space-y-2">
                   <div className="flex justify-between items-end">
                      <span className="text-[9px] font-black text-stone-500 uppercase">Wzrost</span>
                      <span className="text-xl font-black italic text-white">{profile.bio.height} <small className="text-xs text-stone-600">cm</small></span>
                   </div>
                   <input type="range" min="140" max="220" step="1" value={profile.bio.height} onChange={(e) => setProfile({...profile, bio: {...profile.bio, height: parseFloat(e.target.value)}})} className="w-full h-2 bg-[#0a0a0b] rounded-full appearance-none accent-[#ff7a00] cursor-pointer" />
                </div>
                <div className="space-y-2">
                   <div className="flex justify-between items-end">
                      <span className="text-[9px] font-black text-stone-500 uppercase">Wiek</span>
                      <span className="text-xl font-black italic text-white">{profile.bio.age} <small className="text-xs text-stone-600">lat</small></span>
                   </div>
                   <input type="range" min="15" max="90" step="1" value={profile.bio.age} onChange={(e) => setProfile({...profile, bio: {...profile.bio, age: parseFloat(e.target.value)}})} className="w-full h-2 bg-[#0a0a0b] rounded-full appearance-none accent-[#ff7a00] cursor-pointer" />
                </div>
              </div>
              
              <div className="space-y-4 pt-4 border-t border-white/5">
                  <label className="text-[9px] font-black text-stone-500 uppercase block text-center tracking-widest">Poziom Aktywności</label>
                  <div className="grid grid-cols-5 gap-2">
                    {[ { val: 1.2, label: 'Brak' }, { val: 1.375, label: 'Lekka' }, { val: 1.55, label: 'Średnia' }, { val: 1.725, label: 'Duża' }, { val: 1.9, label: 'Max' } ].map(opt => (
                      <button key={opt.val} onClick={() => setProfile({...profile, bio: {...profile.bio, activity: opt.val}})} className={`py-4 rounded-2xl text-[8px] font-black uppercase transition-all flex flex-col items-center justify-center gap-1 ${profile.bio.activity === opt.val ? 'bg-[#ff7a00] text-black shadow-lg shadow-orange-500/10 scale-105' : 'bg-[#0a0a0b] text-stone-600 border border-white/5'}`}>
                        <span>{opt.label}</span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="space-y-6 pt-4 border-t border-white/5">
                  ...
                  <div className="flex bg-[#0a0a0b] p-1 rounded-2xl border border-white/5">
                      <button 
                        onClick={() => setStrategy('cut', -300)} 
                        className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase transition-all ${profile.goals.currentGoal === 'cut' ? 'bg-[#ff7a00] text-black' : 'text-stone-600'}`}
                      >
                        Redukcja
                      </button>
                      <button 
                        onClick={() => setStrategy('maintain', 0)} 
                        className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase transition-all ${profile.goals.currentGoal === 'maintain' ? 'bg-[#ff7a00] text-black' : 'text-stone-600'}`}
                      >
                        Utrzymanie
                      </button>
                      <button 
                        onClick={() => setStrategy('bulk', 300)} 
                        className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase transition-all ${profile.goals.currentGoal === 'bulk' ? 'bg-[#ff7a00] text-black' : 'text-stone-600'}`}
                      >
                        Masa
                      </button>
                  </div>
                  ...
                  <div className="px-2 space-y-2">
                        <div className="flex justify-between items-end">
                            <span className="text-[8px] font-black text-stone-500 uppercase">Korekta Manualna</span>
                            <span className="text-[#ff7a00] font-black italic">{profile.goals.correction > 0 ? '+' : ''}{profile.goals.correction} kcal</span>
                        </div>
                        <input 
                            type="range" min="-1000" max="1000" step="50" 
                            value={profile.goals.correction} 
                            onChange={(e) => setStrategy('custom' as any, parseInt(e.target.value))} 
                            className="w-full h-2 bg-[#0a0a0b] rounded-full appearance-none accent-[#ff7a00] cursor-pointer" 
                        />
                  </div>
              </div>
            ...
            <section className="bg-[#161618] p-8 rounded-[4rem] border border-[#27272a] space-y-6">
              ...
              {!calculated.isPctValid && <p className="text-red-500 text-[9px] font-black uppercase text-center animate-pulse mt-2 text-center">Błąd: Suma musi wynosić 100% (Jest: {calculated.pctSum}%)</p>}
            </section>
          </div>
        )}
            ...
            <section className="bg-[#161618] p-8 rounded-[3.5rem] border border-[#27272a] shadow-2xl space-y-6 text-center">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {inventory.map((item, idx) => (
                  <div key={idx} className="bg-[#0a0a0b] px-4 py-2 rounded-full border border-white/10 text-[9px] font-bold flex gap-2 items-center text-center">
                    <span className="text-stone-300">{item.name}</span><span className="text-[#ff7a00]">{item.weight}g</span>
                    <button onClick={() => setInventory(inventory.filter((_, i) => i !== idx))} className="text-red-500 ml-1">×</button>
                  </div>
                ))}
              </div>
              ...
              <div className="flex overflow-x-auto gap-2 pb-2 px-2 no-scrollbar">
                  {CATEGORIES.map(cat => (
                      <button 
                        ...
                        className={`px-4 py-2 rounded-full text-[9px] font-black uppercase whitespace-nowrap transition-all border border-white/5 ${inventoryFilter === cat ? 'bg-[#ff7a00] text-black' : 'bg-[#161618] text-stone-500'}`}
                      >
                          {cat}
                      </button>
                  ))}
              ...
              {filteredRecipes.map((recipe, idx) => (
                <details key={idx} className="bg-[#161618] rounded-[2.5rem] border border-[#27272a] overflow-hidden group">
                  <summary className="p-6 flex items-center justify-between cursor-pointer list-none outline-none text-left">
                    <div className="space-y-1 text-left flex-1 mr-4">
                      <div className="flex items-center gap-2 mb-1">
                          <span className="text-[8px] font-black uppercase bg-[#0a0a0b] px-2 py-1 rounded text-stone-500 border border-white/5">{recipe.category || "Inne"}</span>
                  ...
                  <div className="px-8 pb-8 space-y-4 border-t border-white/5 pt-4 text-[11px] text-stone-400 text-left leading-relaxed">
                    ...
                    {recipe.imageUrl && (
                        <div className="w-full h-48 bg-[#0a0a0b] rounded-2xl overflow-hidden mb-4 border border-white/5 relative group/img">
                             <img src={recipe.imageUrl} alt={recipe.name} className="w-full h-full object-cover transition-transform duration-700 group-hover/img:scale-110" />
                             <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-4">
                                 <span className="text-[#ff7a00] font-black text-[10px] uppercase tracking-widest">Wizualizacja AI</span>
                             </div>
                        </div>
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-4">
                        {[
                            {l:'Kcal', v: recipe.kcal}, {l:'B', v: recipe.protein}, {l:'T', v: recipe.fat}, {l:'W', v: recipe.carbs}
                        ].map((m, i) => (
                            <div key={i} className="bg-[#0a0a0b] p-2 rounded-xl text-center border border-white/5">
                                <span className="block text-[8px] text-stone-500 font-bold">{m.l}</span>
                                <span className="block text-xs font-black text-white">{m.v}</span>
                            </div>
                        ))}
                    </div>

                    <p className="font-black text-stone-500 italic">Składniki:</p>
                    <ul className="list-disc pl-4 mb-2">{recipe.ingredients?.map((ing: string, i: number) => <li key={i}>{ing}</li>)}</ul>
                    <p className="font-black text-stone-500 italic">Wykonanie:</p>
                    {recipe.instructions?.map((s: string, i: number) => <p key={i} className="mb-1">{i+1}. {s}</p>)}

                    <button 
                        onClick={() => setPlanModal({show: true, recipe})}
                        className="w-full bg-[#ff7a00]/10 text-[#ff7a00] py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest border border-[#ff7a00]/20 mt-4 hover:bg-[#ff7a00] hover:text-black transition-all"
                    >
                        Dodaj do planu +
                    </button>
                  </div>
              ...
              <div className="bg-[#161618] w-full max-w-sm rounded-[3rem] p-8 border border-[#ff7a00]/20 space-y-6 relative">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <button onClick={() => handleAddToPlan(getTomorrow(), -1)} className="bg-[#0a0a0b] py-4 rounded-2xl border border-white/10 hover:border-[#ff7a00] text-xs font-black uppercase transition-all">
                             Jutro (Nowy slot)
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                          {[0,1,2,3].map(i => (
                              <button key={i} onClick={() => handleAddToPlan(getToday(), i)} className="bg-[#0a0a0b] py-3 rounded-xl border border-white/5 hover:border-[#ff7a00] text-[10px] font-black text-stone-300">
                                  #{i+1}
                              </button>
                          ))}
                      </div>
